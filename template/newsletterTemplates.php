...
<div class="wrap subsciberDiv">
    <h2>Templates</h2>
	<h2><a  class="addNewTemplatesNewsletter"href="<?=admin_url() . '?page=newsletter&action=editTemplates&subaction=create'?>" >Add New</a></h2>
    <table class="subscriberList">
		...
		foreach ($newsletter as $news):?>
            ...
            <tr class="table-row">
                ...
                <td class="col col-7"><a href="<?=admin_url() . '?page=newsletter&action=editTemplates&newsId=' . $news->getId()?>" class='btn btn-sm btn-info updateUser subscriberUpdate'  >Update</a>-
                    <a href="<?=admin_url() . '?page=newsletter&action=deleteTemplates&newsId=' . $news->getId()?>" class='btn btn-sm btn-danger deleteUser subscriberDelete' >Delete</a> </td>
            </tr>
		<?php
			$i++;
		endforeach; ?>
